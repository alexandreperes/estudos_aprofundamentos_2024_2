...
Aula com o [Dr. Alexandre Jaloto](http://lattes.cnpq.br/1670568115393078) sobre **Teoria de Resposta ao Item**.
...
Faça o *download* do arquivo **`banco2PL.csv`** em <https://drive.google.com/file/d/1Eqxb3Pg-qv2m4GnrGqCb6EiKN9oUfgFU/view?usp=sharing>
...
escore.2pl <- fscores(fit.2pl, full.scores.SE = TRUE)
head(escore.2pl)
...
Faça o *download* do arquivo **`bancograded.csv`** em <https://drive.google.com/file/d/1EmtFHcfk8rlV23tVmdu6UfD4ZUfrR8_A/view?usp=sharing>

```{r message=FALSE, warning=FALSE}
library(readr)
banco.graded <- read_delim("dados/bancograded.csv",
                           delim = ";",
                           escape_double = FALSE, trim_ws = TRUE)
```
...
apply(banco.graded, 2, table)

# calibrar os itens no modelo logístico de dois parâmetros
fit.graded <- mirt(data = banco.graded, model = 1, itemtype = 'graded')

# verificar os parâmetros dos itens
coef(fit.graded, IRTpars = TRUE, simplify = TRUE)

# armazenar os parâmetros em um objeto
pars.graded <- data.frame(coef(fit.graded, IRTpars = TRUE, simplify = TRUE)$items)
pars.graded

# o Item_4 possui parâmetro a muito baixo. Vamos ver como estão seus gráficos mais à frente
...
# curba de informação de cada item
...
# calibrar os itens retirando o Item_4
fit.graded2 <- mirt(data = banco.graded[,-4], model = 1, itemtype = 'graded')

# armazenar os parâmetros em um objeto
pars.graded2 <- data.frame(coef(fit.graded2, IRTpars = TRUE, simplify = TRUE)$items)
pars.graded2 # todos os itens possuim discriminação maior do que 0.7

# curvas
plot(fit.graded2, type = 'info')
plot(fit.graded2, type = 'rxx')
plot(fit.graded2, type = 'SE')
plot(fit.graded2, type = 'infoSE')
plot(fit.graded2, type = 'infotrace')
plot(fit.graded2, type = 'trace')

# calcular o escore dos sujeitos e o erro de medida
escore.graded <- fscores(fit.graded2, full.scores.SE = TRUE)
head(escore.graded)
...
# gerar uma tabela com os parâmetros b
pars.graded2
tab.b <- data.frame(pars.graded2[,-1])
tab.b

# gerar gráfico (atenção: só os escores devem ser usados)
head(escore.graded)
wrightMap(thetas = escore.graded[,1], thresholds = tab.b)

# alterar as cores
cores <- matrix(rep(c('darkgreen', 'red', 'darkblue'), 13), byrow = TRUE, ncol = 3)

wrightMap(escore.graded[,1], tab.b,
          show.thr.lab = FALSE,
          thr.sym.col.fg = cores)

# alterar os símbolos
simbolos <- matrix(c(rep(16, 13), rep(17, 13), rep(18, 13)),
                   byrow = FALSE, ncol = 3)

# às vezes a figura é gerada por cima da anterior e a função abaixo elimina as figuras geradas
dev.off()

wrightMap(escore.graded[,1],
          tab.b,
          show.thr.lab = FALSE,
          thr.sym.col.fg = cores,
          thr.sym.col.bg = cores,
          thr.sym.pch = simbolos,
          axis.logits = 'Altura',
          main.title = "Mapa de itens e pessoas",
          axis.persons = "Respondentes",
          axis.items = "Itens",
          thr.sym.cex = 1.5)


# gerar mapa na escala da altura ----

# para entender melhor o procedimento, veja a transformação que é feita no Enem em https://download.inep.gov.br/publicacoes/institucionais/avaliacoes_e_exames_da_educacao_basica/enem_procedimentos_de_analise.pdf
# veja também explicação mais detalhada de transformações de escala em https://preprints.scielo.org/index.php/scielo/preprint/view/5339

# constantes para (0,1) - média e desvio dos escores no mirt (não é 0 e 1 de fato)
m <- mean(escore.2pl[,1])
s <- sd(escore.2pl[,1])

# constantes para escala da altura - média e desvio da altura das pessoas que responderam o questionário
alt.media <- 1.70
alt.dp <- 0.10

# parâmetros na escala da altura
tab.b.altura = round (((tab.b-m)/s) * alt.dp + alt.media, 2)
escore.altura <- round (((escore.graded[,1]-m)/s) * alt.dp + alt.media, 2)

dev.off()
wrightMap(escore.altura, tab.b.altura, show.thr.lab = FALSE,
          thr.sym.col.fg = cores,
          thr.sym.col.bg = cores,
          thr.sym.pch = simbolos,
          axis.logits = 'Altura',
...
# também é possível ordenar os itens pelo b1
tab.b.order <- tab.b[order(tab.b$b1),]
wrightMap(escore.graded[,1],
          tab.b.order,
          show.thr.lab = FALSE,
          thr.sym.col.fg = cores,
...
tab.b.altura.order <- tab.b.altura[order(tab.b.altura$b1),]
wrightMap(escore.altura, tab.b.altura.order, show.thr.lab = FALSE,
          thr.sym.col.fg = cores,
          thr.sym.col.bg = cores,
          thr.sym.pch = simbolos,
          axis.logits = 'Altura',
...
fit.2pl.mg <- multipleGroup(data = banco.2pl, model = 1, group = c(rep('1', 250), rep('2', 250)), itemtype = '2PL', invariance = c('free_mean', 'free_var', colnames(banco.2pl)))
...
DIF(MGmodel = fit.2pl.mg, which.par = c('a1', 'd'), scheme = 'drop') # Deixar o Item_2 livre melhora o modelo de forma estatisticamente significativa
RMSD_DIF(pooled_mod = fit.2pl.mg) # o valor do Item_2 é discrepante

# deixar o Item_8 livre também melhora o modelo (AIC reduz), porém não é significativo e o RMSD não ficou tão alto

# conclusão: o ITem_2 possui DIF

# calibrar deixando o Item_2 livre entre os grupos
fit.2pl.mg2 <- multipleGroup(data = banco.2pl, model = 1, group = c(rep('1', 250), rep('2', 250)), itemtype = '2PL', invariance = c('free_mean', 'free_var', colnames(banco.2pl)[-2]))
pars.mg2 <- coef(fit.2pl.mg2, IRTpars = TRUE, simplify = TRUE)
pars.mg2

# análise de DIF (lembrando que o Item_2 está livre)
DIF(MGmodel = fit.2pl.mg2, which.par = c('a1', 'd'), items2test = c(1, 3:10), scheme = 'drop')
RMSD_DIF(pooled_mod = fit.2pl.mg2)
```

# 14 de novembro

Nessa aula aprendemos Análise Fatorial Exploratória (AFE). Explicar o que é AFE e para o quê ela serve.

## Banco de dados

Usamos o banco de dados `Ten-Item Personality Inventory-(TIPI) (respostas).xlsx`. O banco está disponível em <https://docs.google.com/spreadsheets/d/1PI1oWW-PUHb9hRSlv9rAMyAsEmyqNRrL/edit?usp=sharing&ouid=118205451649792182527&rtpof=true&sd=true>

```{r}
library(readxl)
dados <- read_excel("dados/Ten-Item Personality Inventory-(TIPI) (respostas).xlsx")
dados <- dados[,2:11]
```
...
```{r}
corr <- psych::polychoric(dados)
corr.table <- as.data.frame(corr$rho)
...
```{r}
paleta <- colorRampPalette(c("darkred", "white", "darkblue"))

corrplot::corrplot(corr = as.matrix(corr$rho),
                   method = "color",
                   type = "lower",
                   outline = TRUE,
                   order = "hclust",
...
corrr::network_plot(corr$rho,
                    repel = TRUE,
                    colours = c("red4", "red3", "red2",
                                "white",
                                "blue2", "blue3", "blue4"))
```
...
```{r}
psych::fa.parallel(x = corr$rho,
                   fa = "fa",
                   fm = "minres",
                   n.obs = nrow(dados))
```

**`Continua em 28 de novembro...`**
...
-   Opções para marcadores/símbolos (pch) para usar nos gráficos do R: <https://r-charts.com/base-r/pch-symbols/>

-   Opções de cores para usar nos gráficos no R: <https://r-graph-gallery.com/42-colors-names.html>
...
-   **pBis e bis ambas baixas (ex.: \< 0.20):** Sinal de que o item tem baixa discriminação. Esses itens não contribuem significativamente para a precisão do teste e podem precisar de revisão ou remoção. Itens com baixa pBis e bis podem ser ineficazes e devem ser analisados em termos de conteúdo e clareza para melhorar sua discriminação.
